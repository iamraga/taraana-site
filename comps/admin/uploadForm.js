...
export default function UploadForm({ albumId }) {
    ...
    async function uploadImages({ file: uploadedFile, onSuccess, onProgress, onError }) {
        ...
        new Compressor(uploadedFile, {      
            ...
            success: (resizedFile) => {
                console.log("inside")
                const storageRef = ref(storage, 'images/' + (resizedFile.name + "##" + imageNameTimestamp)); //Adding timestamp to prevent duplicates
                ...
                uploadTask.on('state_changed', (snap) => {
                        ...
                        addDoc(imagesCollRef, { name: resizedFile.name, imageNameinStorage: (resizedFile.name + "##" + imageNameTimestamp), url, createdAt: serverTimestamp() })
                            .then(function(newDocRef){
                                onSuccess({documentId: newDocRef.id, storageName: (resizedFile.name + "##" + imageNameTimestamp), url, status: "ok"});
                            })
